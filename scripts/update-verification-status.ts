...
async function getCustomerVerificationStatus(customerId: number, orderMeta: any[] = []): Promise<boolean> {
    // Note: We can't easily check for image existence in orderMeta without parsing the complex _mnsfpt_user_authenticate field if it exists there.
    // But usually verification status is enough for quick check, BUT user asked for strict check including image.
    // So we MUST fetch customer data to be sure about the image.
    ...
    try {
        ...
        if (authMeta && authMeta.value) {
            ...
            if (fields && fields['mnsfpt_field_7_1']) {
                hasImage = true;
            }
        }
        ...
        return isStatusVerified && hasImage;
    } catch (error) {
