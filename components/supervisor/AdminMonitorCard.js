      ...
      <div className="flex justify-between items-start">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {/* دکمه تغییر وضعیت دستی توسط سوپروایزر */}
            <button 
                onClick={async (e) => {
                    e.stopPropagation();
                    if(!confirm(`آیا وضعیت ${admin.display_name} را تغییر می‌دهید؟`)) return;
                    // TODO: Implement API for supervisor to toggle other users
                    // For now, we can't easily toggle others without a new API endpoint or modifying the existing one.
                    // Let's assume we will add a query param or body param to the toggle endpoint.
                    try {
                        await fetch('/api/admin/toggle-status', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ userId: admin.id, forceStatus: !isOnline })
                        });
                    } catch(e) { console.error(e); }
                }}
                className="mt-1 text-[10px] bg-gray-100 hover:bg-gray-200 px-2 py-0.5 rounded text-gray-600 cursor-pointer"
            >
                {isOnline ? 'تغییر به آفلاین' : 'تغییر به آنلاین'}
            </button>
          </div>
        </div>
      ...
      <div className="space-y-2">
        {slots.map((order, index) => (
          <div key={index} className={`
             relative items-center p-2 rounded-xl border transition-all min-h-[60px]
             ${order ? 'flex bg-white border-indigo-100 shadow-sm' : 'hidden md:flex bg-gray-100/50 border-gray-200 border-dashed'}
          `}>
             {order ? (
