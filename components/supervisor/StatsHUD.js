  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        // Fetch online customers from live-status
        const liveRes = await fetch('/api/supervisor/live-status');
        const liveData = await liveRes.json();
        if (liveData.success && liveData.onlineCustomers !== undefined) {
            setOnlineCustomers(liveData.onlineCustomers);
        }
      } catch (error) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 flex items-center justify-between">
        <div>
          <p className="text-gray-500 text-xs font-bold mb-1">مشتریان آنلاین</p>
          <p className="text-2xl font-black text-indigo-600">{onlineCustomers}</p>
        </div>
