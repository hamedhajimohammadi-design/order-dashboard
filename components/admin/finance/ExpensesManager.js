...
export default function ExpensesManager() {
  ...
  const handleMarkAsPaid = async (id) => {
    if (!confirm('آیا از پرداخت این هزینه اطمینان دارید؟ تاریخ پرداخت به امروز تغییر خواهد کرد.')) return;
    
    try {
      const res = await fetch('/api/finance/expenses', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ id, is_paid: true, date: new Date().toISOString() })
      });
          ...
          <tbody className="divide-y">
            {expenses.map(exp => (
              <tr key={exp.id}>
                ...
                <td className="py-3">
                  {exp.is_paid ? (
                    <span className="bg-green-100 text-green-700 px-2 py-1 rounded-lg text-xs font-bold">پرداخت شده</span>
                  ) : (
                    <span className="bg-red-100 text-red-700 px-2 py-1 rounded-lg text-xs font-bold">پرداخت نشده</span>
                  )}
                </td>
                <td className="py-3">
                  {!exp.is_paid && (
