...
export default function OrderCard({ order }) {
  ...
  const isUserOnline = () => {
    if (!order.last_user_online) return false;
    const lastOnline = new Date(order.last_user_online);
    const now = new Date();
            ...
            <div className="flex flex-col gap-1 mt-1">
                ...
                <div className="flex flex-wrap items-center gap-2 mt-1">
                    <span className="text-xs font-medium text-gray-700 flex items-center gap-1">
                        ...
                        {isUserOnline() && (
                          <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" title="آنلاین"></span>
                        )}
                    </span>
