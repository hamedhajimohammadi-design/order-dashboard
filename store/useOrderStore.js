...
export const useOrderStore = create(
  persist(
    (set, get) => ({
      ...
      fetchAllOrders: async (query = "", page = 1, sortBy = "order_date", sortOrder = "desc") => {
        set({ isSearching: true });
        try {
            // اضافه کردن پارامتر fromToday=true برای فیلتر کردن سفارش‌های قدیمی
            const url = query 
                ? `/api/orders/search?q=${encodeURIComponent(query)}&page=${page}&sortBy=${sortBy}&sortOrder=${sortOrder}` 
                : `/api/orders?status=all&page=${page}&limit=50&fromToday=true&sortBy=${sortBy}&sortOrder=${sortOrder}`;
      ...
      reserveOrder: async (orderId) => {
        ...
        if (order) {
          // Allow waiting orders OR processing orders that are in the pending list (implying no operator)
          ...
          try {
            ...
            if (!res.ok) {
                ...
                alert(errorData.message || "⛔ خطا: این سفارش توسط همکار دیگری رزرو شده است.");
                // رفرش کردن لیست برای حذف سفارش از لیست انتظار
                get().fetchOrders();
                return;
            }

            // Update UI only after success
            const updatedOrder = { ...order, status: 'processing', operator_name: get().currentUser?.display_name || 'Me' };

            set({
              activeOrders: [...activeOrders, updatedOrder],
