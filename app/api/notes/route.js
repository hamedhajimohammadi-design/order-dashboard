...
export async function GET(req) {
  ...
  const idParam = searchParams.get('id'); // WC Order ID or DB ID

  if (!idParam) return NextResponse.json([]);

  let wcOrderId = idParam; 
  let orderInfo = null;
  let dbLogs = [];
  let source = 'pgem'; // Default source

  // 1. Fetch DB Order First to find Source & Info
  try {
    ...
    // If not found, try by ID (maybe idParam was internal ID)
    if (!order) {
       const idAsInt = parseInt(wcOrderId);
       if (!isNaN(idAsInt)) {
    ...
    if (order) {
        orderInfo = order;
        dbLogs = order.logs;
        source = order.source || 'pgem'; // Get source from DB
        wcOrderId = order.wp_order_id.toString(); // Ensure we have correct WC ID
    }
  } catch(e) { 
      console.error("DB Fetch Error:", e); 
  }

  // 2. Fetch WC Notes using correct credentials
  const { siteUrl, key, secret } = getWCCredentials(source);
  
  // 🔴🔴 کوکی ثابت برای دور زدن کش/کلودفلر 🔴🔴
  ...
  wcNotes.forEach(note => {
      const cleanNote = note.note ? note.note.replace(/<[^>]*>?/gm, '') : '';
...
export async function POST(req) {
  try {
    ...
    if (!orderId || !note) return NextResponse.json({ error: 'Missing orderId or note' }, { status: 400 });
